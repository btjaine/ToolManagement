...
{
public:
	static int MessageBox(HWND hParent, LPCTSTR lpstrTitle, LPCTSTR lpstrMsg,bool bIsHideNoButton=false)
	{
		...
		{
			//单个的宽度
			CControlUI* pControl_TC = static_cast<CControlUI*>(pWnd->m_pm.FindControl(_T("LS_TCKD")));
			if (pControl_TC) pControl_TC->SetFixedWidth(200);
